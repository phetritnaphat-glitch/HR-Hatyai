...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-xs">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-13 sm:h-14 flex items-center justify-between gap-2 sm:gap-4">
        
        {/* Left: Hamburger Menu Button + Brand / Logo */}
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <button
            onClick={onToggleMobileMenu}
            className="p-2 -ml-1 text-slate-700 hover:text-emerald-700 hover:bg-emerald-50 rounded-xl md:hidden flex items-center justify-center transition border border-slate-200 shadow-xs"
          ...
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-emerald-600 to-teal-500 flex items-center justify-center text-white shadow-md shadow-emerald-600/20 flex-shrink-0">
            <Building className="w-5 h-5" />
          </div>
          <div className="truncate">
            <div className="flex items-center gap-1.5">
              <h1 className="font-extrabold text-slate-900 text-sm sm:text-lg tracking-tight leading-none truncate">
                HR Online
              </h1>
              <span className="px-1.5 py-0.5 bg-emerald-100 text-emerald-800 text-[9px] sm:text-[10px] font-bold rounded-full uppercase tracking-wider hidden sm:inline-block">
                PRO
              </span>
            </div>
        ...
        <div className="hidden md:flex items-center gap-2 bg-slate-50 border border-slate-200/80 rounded-xl px-3.5 h-10 text-xs text-slate-700 shadow-2xs">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <button
            ...
            disabled={isSyncing}
            className={`h-10 px-3 rounded-xl border font-bold text-xs transition flex items-center gap-1.5 cursor-pointer shadow-2xs ${
              isSyncing
          ...
          >
            ...
            <span className="hidden sm:inline whitespace-nowrap">ซิงค์ข้อมูล</span>
          ...
          <div className="relative">
            <button
              ...
              className={`h-10 w-10 rounded-xl transition relative flex items-center justify-center border cursor-pointer shadow-2xs ${
                notifOpen
            ...
            >
              ...
              {pendingLeavesCount > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 bg-rose-500 text-white text-[10px] font-extrabold rounded-full flex items-center justify-center border-2 border-white shadow-xs animate-bounce">
                  {pendingLeavesCount}
                </span>
              )}
            </button>
          ...
          <div className="h-10 bg-slate-100/90 p-1 rounded-xl flex items-center border border-slate-200/80 text-xs font-semibold whitespace-nowrap shadow-2xs">
            <button
              onClick={() => onToggleAdminView(false)}
              className={`h-8 px-2.5 sm:px-3 rounded-lg transition-all flex items-center justify-center gap-1.5 shrink-0 whitespace-nowrap cursor-pointer ${
                !isAdminView
            ...
            {currentUserAccount?.role === 'hr' ? (
              ...
              <span
                ...
                className="h-8 px-2.5 text-slate-400 opacity-60 flex items-center gap-1 text-[11px] cursor-not-allowed"
              >
                <Lock className="w-3 h-3 text-slate-400" />
                <span>HR (จำกัดสิทธิ์)</span>
              </span>
            )}
          </div>
          ...
          <div className="relative">
            <button
              ...
              className="h-10 flex items-center gap-2 px-2 rounded-xl hover:bg-slate-100/80 bg-slate-50/50 border border-slate-200/80 transition text-left cursor-pointer shadow-2xs"
            >
              <img
                src={currentEmployee.avatar}
                alt={currentEmployee.name}
                className="w-7 h-7 rounded-lg object-cover ring-2 ring-emerald-500/30 shrink-0"
              />
              <div className="hidden lg:block leading-tight">
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold text-slate-900 truncate max-w-[120px]">{currentEmployee.name}</span>
                ...
                <div className="text-[10px] text-slate-500 truncate max-w-[130px]">{currentEmployee.position}</div>
              </div>
              <ChevronDown className="w-3.5 h-3.5 text-slate-400 shrink-0" />
            </button>

            {dropdownOpen && (
              <div className="fixed left-3 right-3 top-16 sm:absolute sm:top-full sm:right-0 sm:left-auto sm:w-72 sm:max-w-xs w-auto mt-2 bg-white rounded-2xl shadow-2xl border border-slate-200 py-2.5 z-50 animate-in fade-in zoom-in-95 duration-150">
                {/* User Info Header */}
                <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/80 -mt-2.5 rounded-t-2xl">
                  <div className="font-bold text-slate-900 text-xs truncate" title={currentEmployee.name}>{currentEmployee.name}</div>
                  <div className="text-[11px] text-slate-500 truncate" title={currentEmployee.email}>{currentEmployee.email}</div>
                  <div className="mt-1.5 flex items-center justify-between gap-1">
                    <span className="text-[10px] text-slate-400 shrink-0">รหัส: {currentEmployee.code}</span>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md truncate shrink-0 ${
                      currentUserAccount?.role === 'hr' ? 'bg-slate-900 text-emerald-400' : 'bg-emerald-100 text-emerald-800'
                    }`}>
                      {currentUserAccount?.role === 'hr' ? 'ผู้ดูแลระบบ (HR)' : 'พนักงานทั่วไป (Employee)'}
                    </span>
                  </div>
                </div>
                ...
                <div className="px-2 pt-1">
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-rose-400 hover:bg-rose-500/10 rounded-xl transition flex items-center gap-2 font-bold text-xs cursor-pointer"
                  >
